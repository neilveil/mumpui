...
export default class Main extends React.Component {
  ...
  static init = ({
    ...
    Main._this.setState(
      ...
      () => {
        ...
        if (quality) Main.quality = quality
        if (descent) Main.descent = descent
  ...
  static quality: any = null
  static descent: any = null
  ...
  onCancel = () => {
    if (Main.onCancel) Main.onCancel()
    this.close()
  }
  onDone = async () => {
    if (Main.processing) return
    ...
    if (Main.maxSize && !Main.icon) {
      img = await mutils.compress({
        img,
        maxSize: Main.maxSize,
        onProgress: (progress: any) => this.setState({ progress })
      })
    }
      ...
      <div className='jinie'>
        ...
        <div className='jinie-editor'>
          <div className='jinie-rotation'>
            <input value={this.state.rotation} min={-180} max={180} step={0.1} onChange={this.onRotate} type='range' />
          </div>
